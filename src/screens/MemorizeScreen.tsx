...
export function MemorizeScreen() {
  const phraseId = useAppStore((state) => state.memorizePhraseId);
  const returnView = useAppStore((state) => state.memorizeReturnView);
  ...
  return <MemorizeWizard phrase={phrase} onExit={() => setView(returnView)} />;
}

function MemorizeWizard({ phrase, onExit }: { phrase: Phrase; onExit: () => void }) {
